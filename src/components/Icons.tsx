...
export type IconName =
  | 'home' | 'history' | 'profile' | 'settings'
  | 'menu' | 'close' | 'arrow-back' | 'arrow-right' | 'chevron-down'
  | 'motorcycle' | 'bicycle' | 'car' | 'box'
  | 'location' | 'location-fill' | 'map-pin' | 'navigation'
  | 'phone' | 'mail' | 'chat' | 'whatsapp' | 'twitter' | 'send'
  | 'bell' | 'bell-fill'
  | 'star' | 'star-fill'
  | 'eye' | 'eye-off'
  | 'lock' | 'key' | 'logout'
  | 'search' | 'filter'
  | 'check' | 'check-circle' | 'info' | 'warning' | 'alert'
  | 'shield' | 'trash'
  | 'edit' | 'user' | 'clock' | 'help'
  | 'cash' | 'timer' | 'moon' | 'document'
  | 'cancel' | 'refresh' | 'upload' | 'cloud-off'
  | 'package' | 'package-check'
  | 'rating-empty' | 'rating-fill'
  | 'distance' | 'speed';
...
export function Icon({ name, size = 24, color = 'currentColor', style, strokeWidth = 2 }: IconProps) {
  const props = {
    width: size,
    height: size,
    viewBox: '0 0 24 24',
  ...
  switch (name) {
    ...
    case 'motorcycle':
      return (
        <svg {...props}>
          ...
          <path d="M9 6l1.5 3H5.5L4 11.5c-.5 1 .5 2 1.5 2h1"/>
          <path d="M9 6h6"/>
          <path d="M12 6l3 5.5H9"/>
          <path d="M15 13.5h3"/>
        </svg>
      );
    case 'bicycle':
      return (
        <svg {...props}>
          ...
          <path d="M5.5 17.5 9 9h7l2.5 5.5"/>
          <path d="M9 9l3.5 8.5"/>
          <path d="M17.5 6l-2.5 3H9"/>
        </svg>
      );
    case 'car':
    ...
    case 'package':
      return <svg {...props}><path d="M16.5 9.4l-9-5.19"/><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>;
    case 'package-check':
      return <svg {...props}><path d="m7.5 4.27 9 5.15"/><path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"/><path d="m3.3 7 8.7 5 8.7-5"/><path d="M12 22V12"/><path d="m16 18 2 2 4-4"/></svg>;

    // ── Location ──────────────────────────────────────────────────────────────
    case 'location':
      return <svg {...props}><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>;
    case 'location-fill':
      return <svg {...props} fill={color} stroke="none"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>;
    case 'map-pin':
      return <svg {...props}><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>;
    case 'navigation':
    ...
    // ── Communication ─────────────────────────────────────────────────────────
    case 'phone':
      return <svg {...props}><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.5 2 2 0 0 1 3.6 1.3h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L7.91 9a16 16 0 0 0 6 6l1.06-1.06a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>;
    case 'mail':
    ...
    case 'bell-fill':
      return <svg {...props} fill={color} stroke="none"><path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z"/></svg>;
    ...
    case 'key':
      return <svg {...props}><path d="m21 2-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0 3 3L22 7l-3-3m-3.5 3.5L19 4"/></svg>;
    case 'logout':
    ...
    case 'timer':
      return <svg {...props}><circle cx="12" cy="13" r="8"/><path d="M12 9v4l2 2"/><path d="M5 3 2 6"/><path d="m22 6-3-3"/><path d="M6.38 18.7 4 21"/><path d="M17.64 18.67 20 21"/></svg>;
    case 'moon':
    ...
    case 'upload':
      return <svg {...props}><polyline points="16 16 12 12 8 16"/><line x1="12" y1="12" x2="12" y2="21"/><path d="M20.39 18.39A5 5 0 0 0 18 9h-1.26A8 8 0 1 0 3 16.3"/></svg>;
    case 'cloud-off':
      return <svg {...props}><path d="M22.61 16.95A5 5 0 0 0 18 10h-1.26A8 8 0 0 0 4.94 3.21"/><line x1="2" y1="2" x2="22" y2="22"/></svg>;
    case 'distance':
      return <svg {...props}><path d="M3 3h18"/><path d="M3 21h18"/><path d="M12 3v18"/><path d="M6 9l-3-6"/><path d="M18 9l3-6"/><path d="M6 15l-3 6"/><path d="M18 15l3 6"/></svg>;
    case 'speed':
      return <svg {...props}><path d="M12 12m-1 0a1 1 0 1 0 2 0 1 1 0 1 0-2 0"/><path d="M20.2 8a9 9 0 1 0 1.8 4"/><path d="M12 12l4.3-4.3"/></svg>;
...
// Convenience wrappers for filled variants
export function StarIcon({ filled, size = 24, color = '#ff9800' }: { filled: boolean; size?: number; color?: string }) {
  return <Icon name={filled ? 'star-fill' : 'star'} size={size} color={color} />;
}
